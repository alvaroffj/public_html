                ...
                case 'descargar':
                    ...
                    if($_GET["id_device"] == "0") {
                        ...
                        if($gr->accountID == $this->cp->getSession()->get("accountID")) {
                            ...
                            $rep = $this->edMP->detencionByDevice($ini, $fin, $dev, $_GET["operador"], $_GET["min"]);
                        }
                    } else {
                        ...
                        if($dev->accountID == $this->cp->getSession()->get("accountID")) {
                            $rep = $this->edMP->detencionByDevice($ini, $fin, array($_GET["id_device"]), $_GET["operador"], $_GET["min"]);
                        }
                    }
                    if($rep != null) {
                        ...
                        $objPHPExcel->getProperties()->setCreator("GPSLine")
                                ->setTitle("Reporte de Detencion " . $ini . " - " . $fin)
                                ->setSubject("Reporte de Detencion " . $ini . " - " . $fin)
                                ->setDescription("Reporte de Detencion " . $ini . " - " . $fin);

                        $objPHPExcel->setActiveSheetIndex(0);
                        $objPHPExcel->getActiveSheet()->setTitle('Detencion');
                        $objReader = PHPExcel_IOFactory::createReader('Excel5');
                        $objPHPExcel = $objReader->load("plantilla.xls");

                        $objPHPExcel->getActiveSheet()
                                ->setCellValueByColumnAndRow(5, 2, 'Reporte de Detencion')
                                ->setCellValueByColumnAndRow(5, 3, utf8_encode('Período de tiempo: ') . $fini . " / ".$ffin);
                        $columnas = array("Fecha", "Vehículo", "Patente", "Latitud", "Longitud", "Detencion");
                        $nCol = count($columnas);
                        $rowIni = 7;
                        for($i=0; $i<$nCol; $i++) {
                            $objPHPExcel->getActiveSheet()
                        ...
                        foreach ($rep as $r) {
                            $objPHPExcel->getActiveSheet()
                                ...
                                ->setCellValueByColumnAndRow(6, $rowIni+$i, $r->detencionMAX);
                            $i++;
                        }
                        header('Content-Type: application/vnd.ms-excel');
                        header('Content-Disposition: attachment;filename="reporte_detencion_'.$ini.'_'.$fin.'.xls"');
...
//                    echo "vel: ".$_POST["operador"];
                    ...
                    if($_POST["id_device"] == "0") {
                        ...
                        if($gr->accountID == $this->cp->getSession()->get("accountID")) {
                            ...
                            $rep = $this->edMP->detencionByDevice($ini, $fin, $dev, $_POST["operador"], $_POST["min"]);
                        }
                    } else {
                        ...
                        if($dev->accountID == $this->cp->getSession()->get("accountID")) {
                            $rep = $this->edMP->detencionByDevice($ini, $fin, array($_POST["id_device"]), $_POST["operador"], $_POST["min"]);
                        }
                    }
//                    echo $rep;
                    if($rep != null) {
